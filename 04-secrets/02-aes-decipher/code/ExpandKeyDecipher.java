 ...
 */
public class ExpandKeyDecipher {
    ...
    // Массив, который состоит из битов 32-х разрядного слова и является постоянным для данного раунда
    ...
    public ExpandKeyDecipher(KeyDecipher k) {
        byte row;
        byte column;

        for (row = 0; row < 4; row++) {
            for (column = 0; column < 4; column++) {
                roundKeys[row][column] = k.getKeyValue()[row][column]; // Заполняет начальный ключ
            }
        }

        byte round = 0;
        for (column = 4; column < 44; column++) { // Вычисляет дополнительные ключи
            if (column % 4 == 0) {
    ...
    // Возвращает i-й ключ, сгенерированный алгоритмом расширения ключа
    ...
    // Помещает старый столбец, повернутый на 1 байт, в новый столбец roundKeys
    ...
    // Вычисляет c1 XOR c2 и помещает результат в newColumn roundKeys
    private void xorColumns(int c1, int c2, int newColumn) {
        for (int row = 0; row < 4; row++) {
            int intermediate = roundKeys[row][c1] ^ roundKeys[row][c2];
            roundKeys[row][newColumn] = (short) intermediate;
        }
    }

    // Заменяет все байты в roundKeys newColumn с помощью S-box
    ...
    /*
        Выполняет XORs для newColumn с соответствующим столбцом (i-4) и соответствующим rCon и помещает результат
        в newColumn. Эту операцию следует использовать только в том случае, если значение newColumn кратно 4, как
        часть алгоритма расширения ключа.
     */
    ...
    // Возвращает бит старшего разряда
    public int highNibble(short x) {
        return ((short) 0xf0 & x) >> 4;
    }

    // Возвращает бит младшего разряда
    public int lowNibble(short x) {
        return ((short) 0xf & x);
    }
}
